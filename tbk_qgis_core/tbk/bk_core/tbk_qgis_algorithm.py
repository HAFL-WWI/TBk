...
from tbk_qgis_core.tbk.utility.tbk_utilities import dict_diff
from tbk_qgis_core.tbk.utility.qgis_processing_utility import QgisHandler
from tbk_qgis_core.tbk.utility.persistence_utility import (read_dict_from_toml_file,
...
class TBkAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # tbk_tool_path = os.path.join(settings_path,"python/plugins/tbk_qgis_core")
        ...
        processing.run("TBk_core:TBk postprocess Cleanup", {
        ...
        if vegZoneLayer:
            ...
            processing.run("TBk_core:Optimized Spatial Join", param)
        ...
        if (calc_vhm_hdom_diff):
            start_time_section = time.time()
            processing.run("TBk_core:TBk postprocess Hdom diff", {
        ...
        if (calc_local_densities):
            start_time_section = time.time()
            processing.run("TBk_core:TBk postprocess local density", {
