...
for statistic driven metrics like quantile and the [Gini coefficient](https://en.wikipedia.org/wiki/Gini_coefficient).
...
resolution. This is given when dividing 10m by the resolution of original VHMS
...
The below as input used VHM does matches most the a.m. criteria resp. can be 
processed with little effort to match them. Some ticks to check and achieve point
...
R-external dependencies versions of the libraries linked `sf`-package:
...
Function for mapping bounding box of a spatial object in a iterative map
...
How does the bounding boxes of original VHM of VHM 10m overlap?
...
Check if aggregation factor is a integer:
