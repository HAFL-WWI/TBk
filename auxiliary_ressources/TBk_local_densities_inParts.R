...
# divide stands in small sub-parts
partsize = 10
stands_parts = split(stands, rep(1:partsize, length.out = nrow(stands), each = ceiling(nrow(stands)/partsize)))
# init
statstable_collect <- data.frame()
polys_collect <- data.frame()

#### ensure dir / log ####
if(VERBOSE) print("----------------------------------")
if(VERBOSE) print(paste0("log_time: ", format(Sys.time(), "%Y-%m-%d_%H-%M")))
...
tryCatch(expr = {
  ...
  dir.create(file.path(PATH_OUTPUT), recursive = FALSE, showWarnings = TRUE)
  return(cond)
}, silent = TRUE)

log_filepath = file.path(PATH_OUTPUT, paste0("TBk_local_densities", NAME_SUFFIX_MAIN, paste0("_log_",format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),".txt")))
capture.output(paste(Sys.time(), "START PROCESSING"), file = log_filepath, append=TRUE)
...
for(p in 1:length(stands_parts)) {
  # for(p in 1:1) {
  # for(p in length(stands_parts):1) { # reverse

  # clean up from previous iterations
  remove("polys", "statstable", "stats", "error_and_message", "multipolygon", "new_polys")

  if(VERBOSE) print("------------------------------------------")
  if(VERBOSE) print(paste0("------------ PROCESS PART ", p, "/", partsize, " ----------"))
  if(VERBOSE) print("------------------------------------------")
  capture.output(paste("------------------------------------------"), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), paste0("------------ PROCESS PART ", p, "/", partsize, " ----------"), file = log_filepath, append=TRUE))
  capture.output(paste("------------------------------------------"), file = log_filepath, append=TRUE)


  NAME_SUFFIX = paste0(NAME_SUFFIX_MAIN, "-", p)
  stands = stands_parts[[p]]
  if(VERBOSE) print(paste0("------------ with ", nrow(stands), " stands -------------"))
  if(VERBOSE) print("------------------------------------------")
  capture.output(paste(Sys.time(), paste0("------------ with ", nrow(stands), " stands -------------")), file = log_filepath, append=TRUE)
  capture.output(paste("------------------------------------------"), file = log_filepath, append=TRUE)
  ...
  for (i in 1:nrow(stands)){
    ...
    error_message = tryCatch(expr={
          ...
          if(length(ras_foc[ras_foc==1])>0){
            ...
            if(!is.null(multipolygon)){
              ...
              if((nrow(new_polys) > 0)){
                ...
                # suppress progress output of exact_extract (has it's own progress-bar that is cluttering the console)
                sink(nullfile())
      ...
      capture.output(paste(Sys.time(), paste("Problem with stand: ", i)), file = log_filepath, append=TRUE)
  ...
  capture.output(paste(""), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), "Finished iteration over stands."), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), paste0("Errors: ", n_errors, " (out of ", nrow(stands), " processed)")), file = log_filepath, append=TRUE)
  ...
  if(VERBOSE) print(PATH_OUTPUT)
  capture.output(paste("----------------------------------"), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), paste0("write local density zones to: ")), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), PATH_OUTPUT), file = log_filepath, append=TRUE)
  ...
  tryCatch(expr={
    ...
    capture.output(paste("----------------------------------"), file = log_filepath, append=TRUE)
    capture.output(paste(Sys.time(), paste0("left join attribute rows to input geometries (stands)")), file = log_filepath, append=TRUE)
    capture.output(paste(Sys.time(), paste0(nrow(statstable), " of ", nrow(stands_all), " were processed (check proc_ID, others are NA).")), file = log_filepath, append=TRUE)
    ...
    if(!OVERWRITE_ORIGINAL_TBK){
      ...
      capture.output(paste(Sys.time(), paste0("write input geometries with appendes attributes to folder: ", PATH_OUTPUT)), file = log_filepath, append=TRUE)
    ...
    if(OVERWRITE_ORIGINAL_TBK){
      ...
      capture.output(paste(Sys.time(), paste0("attempt to overwrite input geometries with appendend attributes: ", PATH_SHP)), file = log_filepath, append=TRUE)
  ...
  if(VERBOSE) print("----------------------------------")
  capture.output(paste(Sys.time()), file = log_filepath, append=TRUE)
  capture.output(paste("----------------------------------"), file = log_filepath, append=TRUE)
  capture.output(paste("------------   DONE   ------------"), file = log_filepath, append=TRUE)
  capture.output(paste("----------------------------------"), file = log_filepath, append=TRUE)

  tryCatch(expr={
    if(VERBOSE) print("-------***********************----")
    if(VERBOSE) print(paste0("append results to collection"))
    capture.output(paste("-------***********************----"), file = log_filepath, append=TRUE)
    capture.output(paste(Sys.time(), paste0("append results to collection")), file = log_filepath, append=TRUE)
    statstable_collect = rbind(statstable_collect, statstable)
    polys_collect =  rbind(polys_collect, polys)
    if(VERBOSE) print("-------***********************----")
    capture.output(paste("-------***********************----"), file = log_filepath, append=TRUE)
  }, error=function(cond) {# optional: print alert #
    message(cond)
  })
...
#### write collected parts ####
NAME_SUFFIX = NAME_SUFFIX_MAIN

# write collected polygons
if(VERBOSE) print(paste0("write local density zones to: ", paste0("TBk_local_densities", NAME_SUFFIX, "_allparts.gpkg")))
st_write(st_as_sf(polys_collect), append = FALSE,
         file.path(PATH_OUTPUT, paste0("TBk_local_densities", NAME_SUFFIX, "_allparts.gpkg")))

# write collected stats
tryCatch(expr={
  if(VERBOSE) print("----------------------------------")
  if(VERBOSE) print(paste0("left join attribute rows to input geometries (stands)"))
  if(VERBOSE) print(paste0(nrow(statstable_collect), " of ", nrow(stands_all), " were processed (check proc_ID, others are NA)."))
  capture.output(paste("----------------------------------"), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), paste0("left join attribute rows to input geometries (stands)")), file = log_filepath, append=TRUE)
  capture.output(paste(Sys.time(), capture.output(paste(Sys.time(), paste0("left join attribute rows to input geometries (stands)")), file = log_filepath, append=TRUE)), file = log_filepath, append=TRUE)


  # connect statstable with existing attributetable/geometry (left outer join)
  stands_out = merge(x = stands_all, y = statstable_collect, by.x = "ID", by.y = "stand_ID", all.x = TRUE)

  if(!OVERWRITE_ORIGINAL_TBK){
    if(VERBOSE) print(paste0("write input geometries with appendes attributes to folder: ", PATH_OUTPUT))
    capture.output(paste(Sys.time(), capture.output(paste(Sys.time(), paste0("write input geometries with appendes attributes to folder: ", PATH_OUTPUT)), file = log_filepath, append=TRUE)), file = log_filepath, append=TRUE)

    # write file with new attributes to PATH_OUTPUT
    write_sf(stands_out, file.path(PATH_OUTPUT, paste0(tools::file_path_sans_ext(basename(PATH_SHP)),"_local-densities", NAME_SUFFIX, "_allparts.gpkg") ))
  }
  if(OVERWRITE_ORIGINAL_TBK){
    if(VERBOSE) print(paste0("attempt to overwrite input geometries with appendend attributes: ", PATH_SHP))
    capture.output(paste(Sys.time(), capture.output(paste(Sys.time(), paste0("attempt to overwrite input geometries with appendend attributes: ", PATH_SHP)), file = log_filepath, append=TRUE)), file = log_filepath, append=TRUE)
...
if(VERBOSE) print("------------------------------------------")
if(VERBOSE) print("------------ ALL PARTS DONE   ------------")
if(VERBOSE) print("------------------------------------------")
capture.output(paste(Sys.time()), file = log_filepath, append=TRUE)
capture.output(paste("------------------------------------------"), file = log_filepath, append=TRUE)
capture.output(paste("------------ ALL PARTS DONE   ------------"), file = log_filepath, append=TRUE)
capture.output(paste("------------------------------------------"), file = log_filepath, append=TRUE)
